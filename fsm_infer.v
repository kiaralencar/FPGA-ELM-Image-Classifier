// Modulo da maquina de estados responsavel pela inferencia 
module fsm_infer (
    input wire clk,
    input wire reset,
    input wire start,
...
reg [3:0] state;
reg [27:0] wait_cnt;
reg [9:0] i; // Indice do pixel (0-783) 
reg [6:0] n; // Indice do neuronio oculto (0-127)
reg [3:0] k; // Indice da classe de saida (0-9)

reg mac_enable_d; // sinal que lida com delay da BRAM apresentar o dado 1 ciclo depois de receber o endereço
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        case (state)
            ...
            MAC_H_LAST: begin
                // Ultimo produto pixel[783]*peso[783] acumulado
                cycles <= cycles + 32'd1;
                mac_enable_d <= 1'b0;
                i <= 10'd0;
                state <= ACTIV;
            ...
            MAC_Y_LAST: begin
                // Ultimo produto h[127]*beta[k][127] acumulado
                cycles <= cycles + 32'd1;
                mac_enable_d <= 1'b0;
                n <= 7'd0;
                state <= SAVE_Y;
